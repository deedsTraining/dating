...
main(int argc, char *argv[]) {
  ...
  strcpy(dateshingtotalprob, argv[1]); /* datetrainshing3totalprob */ 
  strcpy(shinglefile, argv[2]); /* mergershing3onlysortuniq */
  strcpy(dateinfile, argv[3]); /* aawmaster */
  strcpy(resultvalprob, argv[4]);  /* dateshingvalprob */
  ...
  for(i=1; i<=numberoffile; i++){ 
	...
	fscanf(currentfileptr, "%s %s %s", shinglefirst, shinglesecond, shinglethird);
