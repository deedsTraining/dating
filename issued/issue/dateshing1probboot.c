...
main(int argc, char *argv[]) {
  ...
  strcpy(shinglefile, argv[1]); /* mergershing1onlysortuniq */
  ...
  for(k=0; k<bootstrapsample; k++){ /* AN OUTER LOOP HERE FOR REPEATING BOOTSTRAP SAMPLING */
	...
	if( (i%RESIDUEtest)==0 && (i%RESIDUEval)!=0 ){
	  ...
	  strcat(currentfilename, shingled); /* printf("currentfilename=%s \n",  currentfilename);*/
	  ...
	  while(!feof(currentshinglefileinptr)){
	    fscanf(currentshinglefileinptr, "%s", dummystring1);
