...
/* a file called "randomnuberfile" was produce using R. It picks randomly (uniform dist)
each of the training documents from a collection of 2608 training documents. For eg. in a vector
of length 2608, the first training doc. may be picked once, the second 3 times, the third 5 
times, the fourth 0 times, etc.... This means the vector has the value bootvector[0]=1, 
bootvector[1]=3, bootvector[2]=5,  bootvector[3]=0, ...., bootvector[2607]=4. Since the
training documents are picked with replacement from a collectiom 2608 item, the sum of 
bootvector (bootvector[0]+bootvector[1]+....+bootvector[2607]) equals 2608. Now such procedure
is carried out 500 times, the number of the bootstrap iteration we have chosen. 
We are producing the random numbers using R instead of the "rand"random generating C function,
since we are producing 2608*500=1,304,000 random numbers(the numbers produced from "rand" 
unfortunately are not really random). Code for producing "randomnumberfile" in R:
    

randomnumvec array will have to be increaed if the bootstrap iteration is increased from 500 
*/ 
...
main(int argc, char *argv[]) {
  ...
  float h, df,sum, Num[400], Den[400], probs[400], probdoc[400], kernelvalue[378];
  float bottomline;
 
  strcpy(shinglefile, argv[1]); /* mergershing2onlysortuniq */
  ...
  df=atoi(argv[6]); /* degree of freedom df */
  strcpy(dateest, argv[7]); /* dateestboot */ 
  ...
  for(k=0; k<bootstrapsample; k++){ /* AN OUTER LOOP HERE FOR REPEATING BOOTSTRAP SAMPLING */
	...
	if( (i%RESIDUEtest)==0 && (i%RESIDUEval)!=0 ){
	  ...
	  while(!feof(currentshinglefileinptr)){
	    fscanf(currentshinglefileinptr, "%s %s", dummystring1, dummystring2);
		...
		fscanf(wordcountfileinptr, "%d", &freq);
		/*printf("Occvector[%d]=%d \n", count, Occvector[count]);*/
		count++;
	      }
		...
		Den[t]=sum;
	      ...
	      for(t=0; t<=maxdate-mindate; t++){
		/*printf("%.20f %.20f %.20f\n", Num[t], Den[t],(float)(Num[t]/Den[t]) );*/
	      }
	      
	      for(t=0; t<=maxdate-mindate; t++){
		if((float)(Num[t]/Den[t])<bottomline){
		  probs[t]=log(bottomline);
		}
		else{
